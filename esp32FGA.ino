...
const char* humidityURL = "https://personalflask.tunnelconnect.cfd/send2";
const char* errorURL    = "https://personalflask.tunnelconnect.cfd/send_error2";
const char* getFuzzyURL = "https://personalflask.tunnelconnect.cfd/get_fuzzyga";
...
void setup() {
  ...
  if (!ledcAttach((int)pwmPin, pwmFreq, (int)pwmResolution)) {
...
void loop() {
  ...
  if (millis() - lastLoopTime >= loopInterval) {
    ...
    if (controlfuzzy != 0) {
        // Set controlfuzzy to 255 for 500ms
    ...
    // Apply actual controlfuzzy
    Wire.beginTransmission(ARDUINO_ADDR);
    Wire.write((byte)controlfuzzy);
    Wire.endTransmission();
    delay(100);
    Wire.beginTransmission(ARDUINO_ADDR);
    Wire.write((byte)0);
